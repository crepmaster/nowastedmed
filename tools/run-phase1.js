...
try {
  nsEntrypointRule = require(path.join(REVIEWER_ROOT, 'lib/rules/ns-entrypoint-sanity'));
  webpackEntryRule = require(path.join(REVIEWER_ROOT, 'lib/rules/webpack-entry-sanity'));
  nsBootstrapRule = require(path.join(REVIEWER_ROOT, 'lib/rules/ns-bootstrap-chain'));
  noTopLevelFirebaseRule = require(path.join(REVIEWER_ROOT, 'lib/rules/no-top-level-firebase-import'));
  noTopLevelRuntimeRule = require(path.join(REVIEWER_ROOT, 'lib/rules/no-top-level-runtime-initialization'));
  glob = require(path.join(REVIEWER_ROOT, 'node_modules/glob'));
} catch (e) {
  console.error('ERROR: Could not load NativeScript Reviewer from:', REVIEWER_ROOT);
  console.error('Make sure nativescript-reviewer is installed at:', REVIEWER_ROOT);
